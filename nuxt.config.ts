...
export default defineNuxtConfig({
  ...
  pwa: {
      ...
      runtimeCaching: [
        {
          ...
          options: {
            ...
            cacheKeyWillBeUsed: async ({ request }) => {
              return `${request.url}?timestamp=${Math.floor(Date.now() / (1000 * 60 * 5))}`; // 5 min cache buckets
            }
          }
        },
        {
    ...
    manifest: {
      ...
      description: process.env.NUXT_APP_DESCRIPTION || 'Connect with reliable runners for your errands. Post tasks or earn by completing errands in your area.',
      theme_color: process.env.NUXT_THEME_COLOR || '#000000',
  ...
  site: {
    ...
    description: process.env.NUXT_APP_DESCRIPTION || 'Mashughuli connects people who need errands done with reliable runners. Post tasks or earn by completing errands in your area.',
    defaultLocale: 'en'
  },
