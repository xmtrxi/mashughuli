...
<template>
  <NotificationsNotificationProvider>
    <div class="flex min-h-screen bg-background transition-colors duration-200">
      <!-- Desktop Sidebar -->
      <aside class="hidden lg:block w-64 border-r bg-card/50 backdrop-blur-sm overflow-y-auto">
      ...
      <div class="flex-1 flex flex-col min-w-0">
        ...
        <template v-if="isMobile">
          <header class="sticky top-0 z-20 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 p-4">
            <div class="flex justify-between items-center">
              <NuxtLink to="/" class="font-bold text-xl text-primary hover:opacity-80 transition-opacity">
                Mashughuli
              </NuxtLink>
