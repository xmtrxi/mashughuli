...
const isRequster = computed(() => {
  ...
  return authStore.user.id == props.errand.requester.id;
});

const formattedBudget = `${formatCurrency(props.errand.budgetMin ?? 0, "Kes")} - ${formatCurrency(props.errand.budgetMax, "Kes")}`;
</script>
<template>
